import XCTest
@testable import Napkeep

final class NapkeepTests: XCTestCase {
    var store: Store!
    ...
    @MainActor func testFreeLimitBlocksAdd() {
        for _ in 0..<Store.freeLimit {
            _ = store.add(Nap(), isPro: false)
        }
        let blocked = store.add(Nap(), isPro: false)
    ...
    @MainActor func testProBypassesLimit() {
        for _ in 0..<Store.freeLimit {
            _ = store.add(Nap(), isPro: true)
        }
        let added = store.add(Nap(), isPro: true)
        XCTAssertTrue(added)
    }
